  ...
  return (
    <div className="wd-people-details position-fixed top-0 end-0 bottom-0 bg-white p-4 shadow w-25">
      ...
      <div className="text-danger fs-4 wd-name"> 
        ...
        {user && editing && (
          <FormControl className="wd-50 wd-edit-name" defaultValue={`${user.firstName} ${user.lastName}`} onChange={(e) => setName(e.target.value)} onKeyDown={(e) => {
      ...
      {editing && (
        <select className="form-select wd-select-role" value={role} onChange={(e) => setRole(e.target.value)}>
      ...
      {editing && (
        <FormControl type="email" className="wd-50 wd-edit-email" defaultValue={`${user.email}`} onChange={(e) => setEmail(e.target.value)} onKeyDown={(e) => {
          if (e.key === "Enter") { saveUser(); } 
        }} />
      )}
      <br />
